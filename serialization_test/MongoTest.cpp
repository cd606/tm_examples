...
#define MY_STRUCT_FIELDS \
    ...
    ((InfoStruct, info))
TM_BASIC_CBOR_CAPABLE_STRUCT(InfoStruct, INFO_STRUCT_FIELDS);
TM_BASIC_CBOR_CAPABLE_STRUCT(MyStruct, MY_STRUCT_FIELDS);
TM_BASIC_CBOR_CAPABLE_STRUCT_SERIALIZE(InfoStruct, INFO_STRUCT_FIELDS);
TM_BASIC_CBOR_CAPABLE_STRUCT_SERIALIZE(MyStruct, MY_STRUCT_FIELDS);
...
int main() {
    ...
    // Create a Document
    {
        ...
        std::string msJsonStr;
        basic::nlohmann_json_interop::Json<MyStruct>(ms).writeToString(&msJsonStr);
        std::cout << msJsonStr << '\n';
        auto doc1 = bsoncxx::from_json(msJsonStr);
        ...
        doc_view = doc1.view();
        element = doc_view["name"];
        assert(element.type() == bsoncxx::type::k_string);
        name = element.get_string().value;  // For C++ driver version < 3.7.0, use get_utf8()
        assert(0 == name.compare("MongoDB"));

        std::cout << bsoncxx::to_json(doc1) << '\n';
        std::cout << bsoncxx::to_json(doc_value) << '\n';
    }
    ...
    // Insert One Document: { "i": 0 }
    {
        ...
        //basic::nlohmann_json_interop::Json<OneItem>({0}).writeToString(&docStr);
        basic::nlohmann_json_interop::Json<OneItem>({0}).toNlohmannJson(docJson);
        auto docBsonData = nlohmann::json::to_bson(docJson);
        auto insert_one_result = collection.insert_one(bsoncxx::document::value {
            reinterpret_cast<std::uint8_t *>(docBsonData.data()), docBsonData.size(), &emptyDeleter
        });
